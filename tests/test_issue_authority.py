...
def contract_fixture(
    *,
    ...
    if include_supersessions:
        documents[0]["prerelease_supersessions"] = documents[0]["prerelease_supersessions"][:1]
    else:
        documents[0]["prerelease_supersessions"] = []
    return documents


def immutable_retirement_contract() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any]]:
    policy, backlog, policy_schema, backlog_schema = contract_fixture()
    checked_policy = json.loads((ROOT / "issue-authority/policy.json").read_text(encoding="utf-8"))
    policy["prerelease_supersessions"] = copy.deepcopy(checked_policy["prerelease_supersessions"][1:])
    return policy, backlog, policy_schema, backlog_schema


def immutable_product_train_files(policy: dict[str, Any]) -> dict[tuple[str, str, str], bytes]:
    successor = next(
...
def install_pipeline_completion_evidence(
    ...
    *,
    ...
    client.trusted_comments[(".github", issue_number)] = [
        {
            "body": fixture["body"],
            "id": 67,
...
class FakeGitHubApi:
    ...
    def successful_workflow_run(
        ...
        identity = (repository, run_id, commit, workflow_path, workflow_name)
        self.workflow_run_requests.append(identity)
        return any(
            actual_repository == repository
            and actual_run_id == run_id
            and actual_commit == commit
            and (workflow_path is None or actual_workflow_path == workflow_path)
            and (workflow_name is None or actual_workflow_name == workflow_name)
            for (
                actual_repository,
                actual_run_id,
                actual_commit,
...
class ContractValidationTest(unittest.TestCase):
    ...
    def test_prerelease_supersession_requires_a_distinct_successor(self) -> None:
        ...
        with self.assertRaisesRegex(AuthorityError, "distinct successor"):
            validate_contract(policy, backlog, policy_schema, backlog_schema)

    def test_prerelease_supersession_issue_chain_cannot_cycle(self) -> None:
        policy, backlog, policy_schema, backlog_schema = contract_fixture(include_supersessions=True)
        policy["prerelease_supersessions"].append(
            {
                ...
                "successor": {"repository": ".github", "number": 59},
            }
        )

        with self.assertRaisesRegex(AuthorityError, "contains a cycle"):
            validate_contract(policy, backlog, policy_schema, backlog_schema)
...
class IssueIntakeTest(unittest.TestCase):
    ...
    def test_immutable_product_train_binds_obsolete_authorities_with_stale_completion(self) -> None:
        policy, _backlog, _policy_schema, _backlog_schema = immutable_retirement_contract()
        retired = [prerelease_authority_issue(number, status="status:triage") for number in (61, 65)]
        for issue in retired:
            issue["labels"].append({"name": COMPLETION_VERIFIED_LABEL})
        issues = {".github": [(issue, []) for issue in retired]}
    ...
    def test_immutable_product_train_retirement_fails_closed_on_changed_bytes(self) -> None:
        ...
        issues = {".github": [(issue, []) for issue in retired]}
        public_files = immutable_product_train_files(policy)
        key = next(iter(public_files))
        public_files[key] += b"\n"

        with self.assertRaisesRegex(AuthorityError, "product-train digest changed"):
            reconstruct_intake(
                policy,
                FakeDiscovery(policy, issues, public_files=public_files),
            )

    def test_activated_issue_successor_can_chain_into_an_immutable_retirement(self) -> None:
        old_policy, _backlog, _policy_schema, _backlog_schema = contract_fixture(include_supersessions=True)
        issue_59 = prerelease_authority_issue(59)
        issue_61 = prerelease_authority_issue(61)
        old_manifest, _inventory = reconstruct_intake(
            old_policy,
            FakeDiscovery(old_policy, {".github": [(issue_59, []), (issue_61, [])]}),
        )
        old_activation = old_manifest["issues"][0]["superseded_by"]["activation"]

        policy = json.loads((ROOT / "issue-authority/policy.json").read_text(encoding="utf-8"))
        issue_59["state"] = "closed"
        issue_59["state_reason"] = "not_planned"
        issue_59["labels"] = [
            {"name": SUPERSEDED_STATUS_LABEL if label["name"] in STATUS_LABELS else label["name"]}
            for label in issue_59["labels"]
        ]
        issue_65 = prerelease_authority_issue(65, status="status:triage")
        issue_61["labels"] = [
            {"name": "status:triage" if label["name"] in STATUS_LABELS else label["name"]}
            for label in issue_61["labels"]
        ]
        for issue in (issue_61, issue_65):
            issue["labels"].append({"name": COMPLETION_VERIFIED_LABEL})
        issues = {".github": [(issue, []) for issue in (issue_59, issue_61, issue_65)]}
        statuses = {
            (".github", old_activation["commit"]): [
                activation_status(old_activation),
            ]
        }
...
class GitHubApiTest(unittest.TestCase):
    ...
    def test_discovery_reads_immutable_product_train_bytes_with_the_job_token(self) -> None:
        client = GitHubDiscovery("job-token")
        response = FakeResponse(b'{"schema":"durable-workflow.product-train/v2"}')
        ...
        with patch("urllib.request.urlopen", return_value=response) as urlopen:
            payload = client.read_file(
                ...
                ".github",
                "a" * 40,
                "product-train/current.json",
            )

        request = urlopen.call_args.args[0]
        self.assertEqual(response.payload, payload)
        self.assertEqual("GET", request.method)
        self.assertEqual("application/vnd.github.raw+json", request.get_header("Accept"))
        self.assertEqual("Bearer job-token", request.get_header("Authorization"))
        self.assertTrue(request.full_url.endswith(f"/contents/product-train/current.json?ref={'a' * 40}"))
...
class MigrationTest(unittest.TestCase):
    ...
    def test_valid_prerelease_supersession_is_terminal_without_false_completion(self) -> None:
        ...
        targets = qualification_targets(qualification_fixture())
        completion = pipeline_completion_fixture()
        documentation_line = next(
            line for line in completion["body"].splitlines() if line.startswith("- `durable-workflow.github.io:main`")
        )
        completion["body"] = completion["body"].replace(documentation_line + "\n", "")
        completion["targets"] = [
            record for record in completion["targets"] if record["repository"] in {".github", "workflow"}
        ]
        install_pipeline_completion_evidence(self.client, completion, targets, issue_number=59)
        ...
        self.assertNotIn(COMPLETION_REQUIRED_LABEL, label_names(retired))
        ...
        self.assertEqual([], self.client.workflow_run_requests)
    ...
    def test_obsolete_beta_authorities_retire_idempotently_on_ordinary_issue_events(self) -> None:
        policy, backlog, _policy_schema, _backlog_schema = immutable_retirement_contract()
        for item in backlog["items"]:
            if item["kind"] == "cross-repository":
                item["kind"] = "feature"
        client = FakeGitHubApi(policy)
        ...
        for issue in retired:
            issue["labels"].append({"name": COMPLETION_VERIFIED_LABEL})
        client.issues[".github"].extend(retired)
        issues = {".github": [(issue, []) for issue in retired]}
        manifest, _inventory = reconstruct_intake(
            policy,
            FakeDiscovery(
            ...
            trigger_label=COMPLETION_VERIFIED_LABEL,
        )
        supersessions = {
            (record["repository"], record["number"]): record["superseded_by"] for record in manifest["issues"]
        }
        self.assertTrue(manifest["trigger"]["approved"])
        for changes in (
            client.label_updates,
            client.milestone_updates,
            client.created_issues,
            client.replacements,
            client.body_updates,
            client.state_updates,
            client.comment_updates,
        ...
        evidence = audit_backlog(
            ...
            backlog,
            client,
            approved_completion_holds={(".github", 61), (".github", 65)},
            prerelease_supersessions=supersessions,
        )

        self.assertEqual("pass", evidence["outcome"])
        self.assertEqual([61, 65], [item["retired"]["number"] for item in evidence["prerelease_supersessions"]])
        self.assertEqual(2, len(client.status_updates))
        for issue in retired:
            self.assertEqual("closed", issue["state"])
            ...
            self.assertNotIn(COMPLETION_REQUIRED_LABEL, label_names(issue))
            self.assertNotIn(COMPLETION_VERIFIED_LABEL, label_names(issue))
            self.assertIn("immutable product train", client.comments[(".github", issue["number"])])
        self.assertEqual([], client.timeline_reads)

        completed_manifest, _inventory = reconstruct_intake(
            policy,
            FakeDiscovery(
                policy,
                {".github": [(issue, []) for issue in retired]},
                commit_statuses=client.commit_statuses,
                public_files=immutable_product_train_files(policy),
            ),
        )
        completed_supersessions = {
            (record["repository"], record["number"]): record["superseded_by"] for record in completed_manifest["issues"]
        }
        self.assertTrue(all(record["completion_evidence_required"] is False for record in completed_manifest["issues"]))
        for changes in (
            client.label_updates,
            client.milestone_updates,
            client.created_issues,
    ...
    def test_production_pipeline_completion_record_stays_closed_with_exact_aggregate_evidence(self) -> None:
        ...
        for record in fixture["targets"]:
            ...
            self.assertIn(f"cited run `{record['qualification_run']}`", current)
    ...
    def test_legacy_generic_completion_uses_the_cited_runs_authenticated_workflow(self) -> None:
        fixture = pipeline_completion_fixture()
        targets = qualification_targets(qualification_fixture())
        selected = [targets[record["repository"]] for record in fixture["targets"]]
        ...
        fixture["body"] = fixture["body"].replace(
            named_line,
            "Required public qualification passed: "
        ...
        source_key = (source["repository"], source["commit"])
        self.client.successful_check_runs[source_key] = {
            check: 30200000901 for check in targets[source["repository"]]["required_checks"]
        }
        ...
        self.assertTrue(
            all(
                requested_path is None and requested_name is None
                for _repository, _run, _commit, requested_path, requested_name in self.client.workflow_run_requests
            )
        )
        source_result = next(
    ...
    def test_split_completion_record_binds_backticked_workflow_and_both_source_commits(self) -> None:
        ...
        completed_line = next(line for line in fixture["body"].splitlines() if line.startswith("Completed in "))
        fixture["body"] = fixture["body"].replace(
            completed_line,
            "Implemented in "
            f"[`{implementation_commit[:12]}`](https://github.com/durable-workflow/.github/commit/"
            f"{implementation_commit}) on `main`.\n"
            "Included by completion source "
            f"[`{source['commit'][:12]}`](https://github.com/durable-workflow/.github/commit/"
            f"{source['commit']}) on `main`.",
        )
        named_line = next(
            line for line in fixture["body"].splitlines() if line.startswith("Required public qualification ")
        )
        fixture["body"] = fixture["body"].replace(
            named_line,
            f"Required public qualification `{source['workflow_name']}` (`{source['workflow_path']}`) "
            f"passed in [run {source['qualification_run']}]"
            f"(https://github.com/durable-workflow/.github/actions/runs/{source['qualification_run']}).",
        ).replace(
            "durable-workflow-implementation-source: " + "d" * 40,
            f"durable-workflow-implementation-source: {implementation_commit}",
        )
        install_pipeline_completion_evidence(self.client, fixture, targets)
        ...
        trusted_comment["body"] = fixture["body"].replace(
            f"durable-workflow-implementation-source: {implementation_commit}",
            "durable-workflow-implementation-source: " + "f" * 40,
        )
        forged_marker = evaluate_lifecycle(
            self.client,
            "durable-workflow",
            ".github",
            {"number": 99},
    ...
    def test_mixed_pull_request_and_protected_branch_completion_preserves_later_regression(self) -> None:
        ...
        self.client.timelines[(".github", 99)] = [closing_reference("workflow", 70)]
        ...
        self.client.successful_checks[("workflow", workflow["commit"])] = set(targets["workflow"]["required_checks"])
        ...
        self.assertTrue(assessment["complete"])
        self.assertEqual("mixed-landing-record", assessment["_authority_kind"])
        by_repository = {target["repository"]: target for target in assessment["targets"]}
        self.assertIn("/pull/70", by_repository["workflow"]["pull_request"])
        self.assertEqual(
            "authenticated-completion-record",
            by_repository[".github"]["provenance"],
        )
        ...
        by_repository = {target["repository"]: target for target in regressed["targets"]}
        self.assertEqual("pending:rejected", by_repository["workflow"]["state"])
        self.assertIn("/pull/70", by_repository["workflow"]["pull_request"])

    def test_later_completion_evidence_hold_reopens_direct_completion(self) -> None:
        for item in self.backlog["items"]:
            if item["kind"] == "cross-repository":
                item["kind"] = "feature"
        apply_backlog(self.policy, self.backlog, self.client)
        ...
        parent = {
            ...
            "labels": [
                {"name": "authority:github"},
                {"name": COMPLETION_REQUIRED_LABEL},
                {"name": "kind:cross-repository"},
                {"name": "priority:P1"},
                {"name": "status:done"},
            ],
        ...
        with self.assertRaisesRegex(
            ...
            audit_backlog(
                ...
                self.backlog,
                self.client,
                approved_completion_holds={(".github", 99)},
                cross_repository_targets={(".github", 99): selected},
            )

        self.assertEqual("open", parent["state"])
        self.assertEqual("reopened", parent["state_reason"])
        self.assertIn("Every declared target landing", self.client.comments[(".github", 99)])
    ...
    def test_pipeline_completion_rejects_spoofed_or_incomplete_records(self) -> None:
        ...
        forged_body = FakeGitHubApi(self.policy)
        install_pipeline_completion_evidence(forged_body, fixture, targets)
        forged_body.trusted_comments.clear()

        ignored = evaluate_lifecycle(
            forged_body,
            "durable-workflow",
            ".github",
            {"body": fixture["body"], "number": 99},
            selected,
            trusted_actors=self.policy["intake"]["trusted_actors"],
        )

        self.assertFalse(ignored["complete"])
        self.assertTrue(all(target["state"] == "pending:no-linked-pull-request" for target in ignored["targets"]))
