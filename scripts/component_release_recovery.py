...
def protected_environment_evidence(
    ...
    if not isinstance(environment, dict):
        raise RecoveryError(
            f"GitHub environment {SUPERSESSION_ENVIRONMENT} has invalid public evidence",
            "plan-discovery",
        )
    ...
    if (
        not rule_ids
        or not reviewer_user_ids
        or type(prevent_self_review) is not bool
...
def validate_continuity_resolution_qualification(
    qualification: Any,
    client: PublicClient,
) -> dict[str, Any]:
    expected = {
    ...
    try:
        run = client.json(
            f"https://api.github.com/repos/{CONTROL_REPOSITORY}/actions/runs/"
            f"{run_id}/attempts/{run_attempt}"
        )
    except NotFound as error:
        raise RecoveryError(
            "continuity successor resolution Beta candidate qualification is absent",
            "plan-discovery",
        ) from error
    ...
    if run.get("head_sha") != qualification["head_sha"]:
        raise RecoveryError(
            "continuity successor resolution qualification is bound to another source revision",
            "plan-discovery",
        )
    if run.get("status") != "completed":
        raise RecoveryError(
            "continuity successor resolution Beta candidate qualification is pending",
            "plan-discovery",
        )
    if run.get("conclusion") == "cancelled":
        raise RecoveryError(
            "continuity successor resolution Beta candidate qualification was cancelled",
            "plan-discovery",
        )
    if run.get("conclusion") != "success":
        raise RecoveryError(
            "continuity successor resolution Beta candidate qualification failed",
            "plan-discovery",
        )
...
def resolve_continuity_successor_fork(
    ...
    if resolution_commit is None:
        raise RecoveryError(
            f"continuity successor resolution {resolution_tag} is absent",
            "plan-discovery",
        )
    resolution = read_record(
        client,
        resolution_tag,
        resolution_commit,
